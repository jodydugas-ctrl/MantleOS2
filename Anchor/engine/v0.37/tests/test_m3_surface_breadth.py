...
class M3SurfaceBreadthTests(unittest.TestCase):
    ...
    def test_non_parser_value_calls_do_not_create_cli_surfaces(self):
        with tempfile.TemporaryDirectory() as tmp:
            td = Path(tmp)
            root = td / "specimen"
            root.mkdir()
            (root / "config.cpp").write_text('''
class Entry { public: QString value(const char *key) const; };
void import(const Entry &lexer) {
    auto name = lexer.value("name");
    auto style = lexer.value("styleID");
}
''', encoding="utf-8")
            out = td / "out"
            summary = ScanEngine().scan(root, out, "non-cli-value@example")
            cli = [
                n for n in summary["nodes"]
                if json.loads(n["attributes_json"]).get("surface_type") == "cli_option"
            ]
            self.assertEqual(cli, [])

    def test_qcommandline_parser_parameter_allows_consumer_reference(self):
        with tempfile.TemporaryDirectory() as tmp:
            td = Path(tmp)
            root = td / "specimen"
            root.mkdir()
            (root / "cli.cpp").write_text('''
#include <QCommandLineParser>
void consume(QCommandLineParser &parser) {
    if (parser.isSet("headless")) {}
}
''', encoding="utf-8")
            out = td / "out"
            summary = ScanEngine().scan(root, out, "cli-parameter@example")
            cli = [
                n for n in summary["nodes"]
                if n["name"] == "--headless"
                and json.loads(n["attributes_json"]).get("surface_type") == "cli_option"
            ]
            self.assertEqual(len(cli), 1)

    def test_plain_ui_object_access_is_reference_evidence_not_a_surface(self):
        with tempfile.TemporaryDirectory() as tmp:
            td = Path(tmp)
            root = td / "specimen"
            root.mkdir()
            (root / "Dialog.cpp").write_text('''
void Dialog::load() {
    ui->setupUi(this);
...
''', encoding="utf-8")
            out = td / "out"
            summary = ScanEngine().scan(root, out, "ui-object-reference@example")
            refs = [n for n in summary["nodes"] if n["kind"] == "ui_object_reference"]
            self.assertEqual({n["name"] for n in refs}, {"setupUi", "statusLabel"})
            store = Store(out / "scan_index.sqlite")
            closure = surface_closure(store)
            store.close()
            self.assertEqual(closure["surface_count"], 0)

    def test_ui_signal_sender_without_declaration_remains_a_surface_candidate(self):
        with tempfile.TemporaryDirectory() as tmp:
            td = Path(tmp)
            root = td / "specimen"
            root.mkdir()
...
#include <QPushButton>
void Dialog::wire() {
    connect(ui->runButton, &QPushButton::clicked, this, &Dialog::run);
}
void Dialog::run() {}
''', encoding="utf-8")
            out = td / "out"
            ScanEngine().scan(root, out, "ui-sender-reference@example")
            store = Store(out / "scan_index.sqlite")
            closure = surface_closure(store)
            store.close()
            run = next(r for r in closure["records"] if r["name"] == "runButton")
            self.assertEqual(run["surface_type"], "ui_object_reference")
            self.assertEqual(run["closure"], "BOUND")
    ...
    def test_file_open_event_routes_to_its_containing_event_override(self):
        with tempfile.TemporaryDirectory() as tmp:
            td = Path(tmp)
            root = td / "specimen"
            root.mkdir()
            (root / "Application.cpp").write_text('''
class QEvent { public: enum Type { FileOpen }; Type type() const; };
class Application { public: bool event(QEvent *event); void openFile(); };
...
''', encoding="utf-8")
            out = td / "out"
            ScanEngine().scan(root, out, "file-open-event@example")
            store = Store(out / "scan_index.sqlite")
            closure = surface_closure(store)
            store.close()
            file_open = next(r for r in closure["records"] if r["surface_type"] == "os_file_open")
            self.assertEqual(file_open["closure"], "BOUND")
            self.assertTrue(any(step["kind"] == "dispatches_to" for step in file_open["example_route"]))
