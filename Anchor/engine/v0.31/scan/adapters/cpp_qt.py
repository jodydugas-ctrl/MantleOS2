...
class CppQtAdapter(Adapter):
    ...
    def extract(self, root: Path, record: FileRecord, text: str) -> ExtractionResult:
        ...
        for m in FILE_OPEN_RE.finditer(text):
            ...
            # A QFileOpenEvent is delivered through a generic QObject::event() override, which is not
            # one of the named pointer/key event methods above. Bind only when this exact evidence is
            # inside an enclosing event() implementation; a mention elsewhere remains unresolved.
            region, fid = enclosing_function(m.start())
            if region is not None and fid is not None and region.name.rsplit("::", 1)[-1] == "event":
                out.edges.append(Edge(
                    stable_id("edge", sid, fid, "dispatches_to"), sid, fid, "dispatches_to", "MAPPED",
                    {"framework_dispatch": True, "event_type": "QEvent::FileOpen"}, [eid],
                ))
