...
class ClangCppAdapter(Adapter):
    ...
    def __init__(self, clang: str | None = None, *, timeout_seconds: float = 5.0,
        ...
        self.version = f"5-{self.tool_version}"
    ...
    def extract(self, root: Path, record: FileRecord, text: str) -> ExtractionResult:
        ...
        cmd = [self.clang, *flags, "-fsyntax-only", "-fno-color-diagnostics", "-Wno-everything", "-Xclang", "-ast-dump=json", str(source)]
        try:
            proc = subprocess.run(cmd, capture_output=True, text=True, timeout=self.timeout_seconds, check=False)
        except subprocess.TimeoutExpired:
            out.findings.append(Finding(
                stable_id("finding", record.id, self.name, "timeout"), "parser_gap",
                f"Compiler AST timed out for {record.path}", "BLOCKED",
                {"parser": "clang", "timeout_seconds": self.timeout_seconds, "compile_context": context}, [],
            ))
            return out
        ...
        diagnostics = (proc.stderr or "").strip().splitlines()
        ast_json_bytes = len((proc.stdout or "").encode("utf-8", errors="replace"))
        if ast_json_bytes > self.max_ast_json_bytes:
            out.findings.append(Finding(
                stable_id("finding", record.id, self.name, "ast-output-too-large", ast_json_bytes,
                          self.max_ast_json_bytes, context["context_digest"]),
                "parser_gap",
                f"Compiler AST output exceeded the safe parse limit for {record.path}",
                "PARTIAL",
                {"parser": "clang", "returncode": proc.returncode, "ast_json_bytes": ast_json_bytes,
                 "max_ast_json_bytes": self.max_ast_json_bytes, "diagnostics": diagnostics[:8],
        ...
        try:
            ...
            state = "PARTIAL" if proc.returncode != 0 else "BLOCKED"
            out.findings.append(Finding(
                stable_id("finding", record.id, self.name, "invalid-json"), "parser_gap",
                f"Compiler AST emitted invalid JSON for {record.path}", state,
                {"parser": "clang", "returncode": proc.returncode, "error": str(exc),
                 "diagnostics": diagnostics[:8], "safe_compile_db_flags": flags,
                 "compile_context": context, "fallback_expected": "cpp_qt"}, [],
            ))
            return out

        parse_coverage = "PARTIAL" if proc.returncode != 0 else "MAPPED"
        if proc.returncode != 0:
            out.findings.append(Finding(
                stable_id("finding", record.id, self.name, "parse-failed", proc.returncode, context["context_digest"]), "parser_gap",
                f"Compiler AST parse incomplete for {record.path}", "PARTIAL",
                {"parser": "clang", "returncode": proc.returncode, "diagnostics": diagnostics[:8],
                 "safe_compile_db_flags": flags, "compile_context": context,
                 "recovered_valid_ast": True, "fallback_expected": "cpp_qt"}, [],
            ))
        ...
        out.nodes.append(Node(
            ...
            {"parser": "clang_ast", "tool_version": self.tool_version, "safe_compile_db_flags": flags,
             "compile_context": context, "returncode": proc.returncode,
             "recovered_after_diagnostics": proc.returncode != 0,
        ...
        def walk_function(node: dict[str, Any], current_scan_id: str | None = None, current_qn: str | None = None,
            ...
            if current_scan_id and node.get("kind") in CALL_KINDS:
                ...
                if line is not None:
                    eid = stable_id("evidence", record.id, self.name, "call", current_qn, callee_qn, signature, line, self.tool_version)
                    out.evidence.append(Evidence(eid, record.id, record.path, line, line, "DIRECT", self.name, excerpt_for(node)))
                    call_id = stable_id("node", record.id, "clang_call", current_qn, line, callee_qn or ref_id or "unknown", signature)
                    out.nodes.append(Node(
                        ...
                        {"callee_name": (callee_qn or "").rsplit("::", 1)[-1] or None,
                         ...
                         "referenced_decl_id": ref_id, "canonical_decl_id": root_ref,
