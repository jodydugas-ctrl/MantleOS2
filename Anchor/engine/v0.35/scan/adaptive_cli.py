...
def _triage_gaps_command(args: list[str]) -> int:
    parser = argparse.ArgumentParser(
        prog="scan-body triage-gaps",
        description="rank unresolved gaps by investigation leverage without changing confidence or canonical state",
    )
    parser.add_argument("db", type=Path)
    parser.add_argument("--out-dir", type=Path, required=True)
    ns = parser.parse_args(args[1:])
    ...
    try:
        coverage = build_coverage_report(store, engine_version=__version__)
        challenges = build_uncertainty_challenges(
            store, engine_version=__version__, coverage_report=coverage,
        )
        result = write_triage_outputs(
            store,
            ns.out_dir,
            engine_version=__version__,
