...
v0.24 adds a bounded second-stage Clang recovery path for translation units whose complete JSON AST exceeds the configured parse ceiling or times out. SCAN now drains compiler stdout and stderr concurrently, stops the compiler when stdout crosses the ceiling, and never retains more than the configured JSON limit in process memory. It then derives one declaration substring from the source filename and mechanically recognized class/function regions, asks Clang for only matching declarations, and accepts the result only when the filtered output is valid and remains below the same ceiling. Filtered evidence is always `PARTIAL`, records the filter and configured execution bounds, preserves the filtered compiler diagnostics, and continues to run conservative fallback extraction. Process-local Clang declaration pointers are used only for in-memory resolution; persisted call identities use stable source location and excerpt evidence instead. Case-equivalent capability-token variants use a total lexical order. This improves source-local compiler evidence without raising the 64 MB safety boundary, claiming a filtered AST is complete, or allowing scheduler timing, process addresses, or set iteration to alter canonical projections.
